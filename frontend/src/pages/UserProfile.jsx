        ...
        <div className="space-y-6">
                    ...
                    <div className="bg-card border border-border rounded-3xl overflow-hidden shadow-sm">
                        ...
                        <div className="px-6 pb-6">
                            ...
                            <div className="flex justify-between items-end -mt-14 mb-4 relative z-10">
                                <div className="w-28 h-28 rounded-3xl bg-card p-1.5 shadow-xl border border-border">
                                ...
                                <div className="flex gap-2">
                                    {isOwnProfile ? (
                                        <>
                                            ...
                                            <Link to="/settings" className="p-2.5 rounded-xl border border-border text-muted-foreground hover:bg-muted hover:text-foreground transition-colors">
                                                <Settings size={20} />
                                            </Link>
                                        </>
                                    ) : (
                            ...
                            <div className="mb-4">
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    <span className={`text-xs font-bold px-2.5 py-1 rounded-lg ${profile.role === 'Admin' ? 'bg-purple-500/10 text-purple-500' :
                                        profile.role === 'Faculty' ? 'bg-amber-500/10 text-amber-500' :
                            ...
                            {profile.profileData?.bio && (
                                <p className="text-foreground/80 mb-5 leading-relaxed">{profile.profileData.bio}</p>
                            )}
                            ...
                            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground border-t border-border pt-4">
                                {profile.profileData?.location && (
                                    <span className="flex items-center gap-1.5">
                                        <MapPin size={16} className="text-muted-foreground" /> {profile.profileData.location}
                                    </span>
                                )}
                                ...
                                <span className="flex items-center gap-1.5">
                                    <Calendar size={16} className="text-muted-foreground" /> Joined {formatDate(profile.createdAt)}
                                </span>
                            </div>
                        </div>
                    </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-1.5 flex gap-1 shadow-sm">
                <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-all ${activeTab === 'posts' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'
                        }`}
                >
                    <Grid size={16} />
                    Posts
                ...
                <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-all ${activeTab === 'about' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:bg-muted'
                        }`}
                >
                    <BookOpen size={16} />
                    About
            ...
            {activeTab === 'posts' ? (
                    ...
                    {posts.length === 0 ? (
                        <div className="bg-card border border-border rounded-3xl p-12 text-center shadow-sm">
                            ...
                            {isOwnProfile && (
                                <Link
                                    to="/create"
                                    className="inline-flex items-center px-5 py-2.5 bg-primary text-primary-foreground rounded-xl text-sm font-semibold hover:bg-primary/90 transition-colors"
                                >
                                    Create Post
                                </Link>
                            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
                        ...
                        <p className="text-foreground leading-relaxed">{profile?.profileData?.bio || 'No bio available'}</p>
                    </div>
                    ...
                    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
                        ...
                        <p className="text-foreground">{profile?.profileData?.location || 'Not specified'}</p>
                    </div>
                    ...
                    {profile?.profileData?.website && (
                        <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
                            ...
                            <a href={profile.profileData.website} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline break-all">
                                {profile.profileData.website}
                            </a>
                        </div>
                    )}
